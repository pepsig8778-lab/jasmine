...
3. Runtime **Python 3**; Build `pip install -r requirements.txt`; Start `python server.py`.
4. **Environment → Add** переменную `SUBITO_PROXY` = *(строка вашего прокси из
   `proxy.txt`, формат `host:port:user:pass`)*
...
| **Разделы (порядок / вид)** | порядок разделов ↑↓, показать/скрыть любой раздел и шапку |
