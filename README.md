...
|---|---|---|
| `GET` | `/api/image?key=&url=&scale=` | **ссылка → PNG** (`image/png`) |
...
Параметры `/api/image`: `key` — ключ, `url` — ссылка subito.it, `scale` — 1–3
(по умолчанию 2). Ошибки — JSON с `error`: **401** ключ, **400** ссылка/не объявление,
**502** Subito недоступен, **500** рендер. `/api/parse` отдаёт те же коды (без 401 — ключ не нужен).
